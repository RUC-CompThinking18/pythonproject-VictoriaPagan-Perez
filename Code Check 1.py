...
total_choice = [random.choice(love) , random.choice(race) , random.choice(religion) , random.choice(depression) , random.choice(Neigh_Bullies) , random.choice(Break_Up)]
...
def poem():
    ...
    for lines in range(length):
        line_type = random.choice(total_choice) #I then told the program to choose  any of the lines in total_choice then to append those to the empty string. I then told the program to return the now full string and then print that out.'''
        chosen.append(line_type)
    return chosen
print poem()
